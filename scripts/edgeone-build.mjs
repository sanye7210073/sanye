...
child.on('exit', (code, signal) => {
  if (!signal && code === 0) {
    ...
    // 生成 .env 文件，确保 Node.js SSR 函数能读取到环境变量
    // EdgeOne Pages 的环境变量在构建时注入，但 SSR 运行时需要 .env 文件
    const envLines = [];
    for (const key of runtimeEnvKeys) {
      if (process.env[key] !== undefined) {
    ...
    const ssrEnvDir = join(process.cwd(), '.edgeone', 'cloud-functions', 'ssr-node');
    if (existsSync(ssrEnvDir)) {
      writeFileSync(join(ssrEnvDir, '.env'), envContent);
      console.log('[edgeone-build] Generated .env for SSR Node.js function');
    }
    // 同时在 .edgeone 根目录生成一份
    writeFileSync(join(process.cwd(), '.edgeone', '.env'), envContent);
    console.log('[edgeone-build] Generated .env in .edgeone root');
  }
